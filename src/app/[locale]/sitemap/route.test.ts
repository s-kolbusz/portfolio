...
async function getSitemapXml(locale: string) {
  const response = await GET(new Request(`https://www.kolbusz.xyz/${locale}/sitemap.xml`), {
...
describe('locale sitemap route', () => {
  it('returns valid XML with content-type text/xml', async () => {
    const response = await GET(new Request('https://www.kolbusz.xyz/en/sitemap.xml'), {
  ...
  it('includes x-default hreflang pointing to en variant for static routes', async () => {
    ...
    expect(xml).toContain('href="https://www.kolbusz.xyz/en"')
    expect(xml).toContain('href="https://www.kolbusz.xyz/en/projects"')
    expect(xml).toContain('href="https://www.kolbusz.xyz/en/cv"')
    expect(xml).toContain('href="https://www.kolbusz.xyz/en/services"')
  })
  ...
  it('includes both locale hreflang tags (en and pl) for each static route', async () => {
    ...
    expect(xml).toContain('href="https://www.kolbusz.xyz/pl/projects"')
  })
  ...
  it('includes x-default for project detail routes', async () => {
    ...
    expect(xml).toContain('href="https://www.kolbusz.xyz/en/projects/zakofy"')
    expect(xml).toContain('href="https://www.kolbusz.xyz/pl/projects/zakofy"')
  ...
  it('x-default always points to the en variant even when served for pl locale', async () => {
    ...
    expect(xml).toContain('hreflang="x-default" href="https://www.kolbusz.xyz/en/projects"')
    expect(xml).toContain('hreflang="x-default" href="https://www.kolbusz.xyz/en"')
  })

  it('includes all static routes in the sitemap', async () => {
    const xml = await getSitemapXml('en')

    expect(xml).toContain('<loc>https://www.kolbusz.xyz/en</loc>')
    expect(xml).toContain('<loc>https://www.kolbusz.xyz/en/projects</loc>')
    expect(xml).toContain('<loc>https://www.kolbusz.xyz/en/cv</loc>')
    expect(xml).toContain('<loc>https://www.kolbusz.xyz/en/services</loc>')
  })
})
